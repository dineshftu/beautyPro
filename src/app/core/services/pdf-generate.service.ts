...
export class PdfGenerateService {
  ...
  getInvoicePdf(data: InvoiceSaveRequest, customer: any) {
    ...
    pdf.text(40, startVertical += 16, `Invoice No : `);
  ...
  getInvoicePdfSource(data: InvoiceSaveRequest) {
    var source = [];
    ...
    for (let index = 0; index < data.products.length; index++) {
      source.push(
        [
          ...
          { content: `${data.products[index].price.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}` }
        ]
      );
    }
    source.push(
    ...
    source.push(
      [
        ...
        { content: '' },
        { content: `${data.discount.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}` }
      ]
    );
    source.push(
      [
        ...
        { content: '' },
        { content: `${(data.productSubTotal + data.treatmentSubTotal - data.discount).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}` }
      ]
    );
